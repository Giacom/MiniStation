...
/obj/item/weapon/paper/generator
	...
	info = "<h2>How to setup the Thermo-Generator</h2><ol>	<li>To the top right is a room full of canisters; to the bottom there is a room full of pipes. Connect C02 canisters to the pipe room's top connector ports.</li>	<li>Connect 3 plasma and 2 oxygen canisters to the bottom ports of the pipe room.</li>	<li>Turn on all the pumps in the room, no adjustments to the pump strength needed.</li>	<li>Look into the camera monitor to see the burn chamber. When it is full of plasma, press the igniter button.</li>	<li>Setup the SMES cells in the North West of Engineering and set an input of half the max; and an output that is half the input.</li></ol>Well done, you should have a functioning generator generating power. If the right side of the generator stops working, it means there is too much pressure and you should try to reverse the pumps, on that side, to suck out the unneeded pressure into the now empty C02 canisters. "

/obj/item/weapon/paper/generator/New()
	..()
	info_links = info